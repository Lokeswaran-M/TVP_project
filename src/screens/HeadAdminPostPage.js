...
const HeadAdminPostPage = ({ navigation }) => {
  ...
  const [datePickerType, setDatePickerType] = useState('start'); // track whether the start or end date picker is open
  const [monthYearVisible, setMonthYearVisible] = useState(false); // Month/Year picker visibility
  const [selectedMonth, setSelectedMonth] = useState(null); // Store selected month and year
